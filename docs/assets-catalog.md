...
- **配置先:** `public/assets/` 配下にカテゴリ別に格納済み（Web表示用に最適化、計7.5MB）
- **元画像（フル解像度）の保管場所:** `C:\Users\dande\Desktop\SE能力可視化アプリ`（ローカル）
- **スタイル共通点:** 青基調・グロッシー（光沢）・ゲームのステータス画面風

> 各画像は用途別に最適化済み（ランク/アイコン=128〜160px、タブ/ボタン=360〜480px、背景=最大1024px、透過保持）。
> アプリからは `/assets/<カテゴリ>/<ファイル名>` で参照できる。
